...
### make these edits to gr00t/experiment/launch_finetune.py

```bash
    config.training.start_from_checkpoint = ft_config.base_model_path
    #config.training.optim = "adamw_torch"
    #config.training.optim = "adafactor"  # More memory-efficient than adamw_torch
    config.training.optim = "paged_adamw_8bit"
    config.training.gradient_checkpointing = True # Trade compute for memory
```
...
```bash
CUDA_VISIBLE_DEVICES=0 uv run scripts/deployment/standalone_inference_script.py \
...
```bash
CUDA_VISIBLE_DEVICES=0 uv run gr00t/eval/open_loop_eval.py \
--dataset-path ../alfiebot_ws/data/alfiebot.CanDoChallenge \
--embodiment-tag NEW_EMBODIMENT \
--model-path ./alfie-gr00t/checkpoint-2000 \
--traj-ids 196 \
...
## TensorRT Mode (0.0849s vs 0.0895s on RTX5090)

### Step 1: Export to ONNX

```bash
CUDA_VISIBLE_DEVICES=0 uv run scripts/deployment/export_onnx_n1d6.py \
...
```bash
CUDA_VISIBLE_DEVICES=0 uv run scripts/deployment/build_tensorrt_engine.py \
  --onnx ./groot_n1d6_onnx/dit_model.onnx \
  --engine ./groot_n1d6_onnx/dit_model_bf16_5090.trt \
  --precision bf16
```

**Output:** `./groot_n1d6_onnx/dit_model_bf16_5090.trt`
